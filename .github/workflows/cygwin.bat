...
%_rootdir%\bin\bash --login -c "ln -s /usr/bin/python3.8 /usr/bin/python && cd D:/a/mys/mys && python -m easy_install pip && python -m pip install -r requirements.txt && python -m pip install pylint && export PYTHONUTF8=1 && make lib && env MYS=""PYTHONPATH=$(pwd) python -m mys"" python -m unittest && ccache -s"
